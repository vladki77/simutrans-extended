...
void vehicle_t::make_smoke() const
{
	...
	if(  smoke  &&  desc->get_smoke()  ) {
		...
		if(  cnv->get_akt_speed() < (sint32)((cnv->get_vehicle_summary().max_sim_speed * 7u) >> 3)  ||  desc->get_engine_type() == vehicle_desc_t::steam  ) {
			...
			if(  gr  ) {
				wolke_t* const abgas = new wolke_t( get_pos(), get_xoff() + ((dx * (sint16)((uint16)steps * OBJECT_OFFSET_STEPS)) >> 8), get_yoff() + ((dy * (sint16)((uint16)steps * OBJECT_OFFSET_STEPS)) >> 8), 8 - get_hoff(), desc->get_smoke() );
				if(  !gr->obj_add( abgas )  ) {
